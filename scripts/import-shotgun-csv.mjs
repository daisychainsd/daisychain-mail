/**
 * Import Shotgun audience emails from a CSV export into Beehiiv.
 *
 * Usage:
 *   BEEHIIV_API_KEY=xxx BEEHIIV_PUBLICATION_ID=pub_xxx \
 *     node scripts/import-shotgun-csv.mjs /path/to/audience.csv
 *
 * Expects the Shotgun CSV format:
 *   First Name,Last Name,Email,Phone,...
 * Finds the "Email" column automatically by header name.
 */
...
async function readEmails(csvPath) {
  ...
  let emailColIndex = 2; // default: Shotgun has Email at index 2
  ...
  for await (const line of rl) {
    ...
    if (isFirstLine) {
      ...
      const idx = headers.findIndex(h => h === "email");
...
async function run() {
  ...
  if (!CSV_PATH) {
    console.error("Usage: node scripts/import-shotgun-csv.mjs /path/to/file.csv");
    process.exit(1);
  }
